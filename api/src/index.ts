...
const verifyTokenViaUserInfo = async (token: string): Promise<Record<string, unknown>> => {
...
app.use(cors({
  origin: [OIDC_CONFIG.frontendUrl, 'http://localhost:5173', 'http://localhost:3000', 'https://phucdh.mindx.com.vn'],
  credentials: true
}));
...
app.get('/auth/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return res.redirect(`${OIDC_CONFIG.frontendUrl}?error=token_exchange_failed`);
    }
...
app.get('/auth/userinfo', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  const authHeader = req.headers.authorization;
  ...
  try {
    ...
    if (!userInfoResponse.ok) {
      res.status(userInfoResponse.status).json({ error: 'Failed to fetch user info' });
      return;
    }
